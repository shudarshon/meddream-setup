     ...
     */
     function parseDicom(byteArray) {
        ...
        function isExplicit(metaHeaderDataSet) {
            ...
            if(transferSyntax.trim() === '1.2.840.10008.1.2') // implicit little endian
            {
                return false;
            }
...
var dicomParser = (function (dicomParser)
{
    ...
    dicomParser.DataSet.prototype.isbyteArray = function(tag, index)
    {
        ...
        if(element && element.length !== 0)
        {
           var tmpByteArray = new Uint16Array(element.length);
            var j = 0;
            for(var i = 0; i < element.length; i+=2)
            {
                tmpByteArray[j] = this.byteArray[element.dataOffset+i] + (this.byteArray[element.dataOffset+i + 1] <<8); // //byteArray[position] + (byteArray[position + 1] << 8);
                j++;
            }
           return tmpByteArray;
        }
    ...
    dicomParser.DataSet.prototype.getTagValue = function(tag, index)
    {
        //AE AS AT CS DA DL DS DT FL FD IS LO LT OB OF OW PN PN SH SL SQ SS ST TM UI UL UN UT  info http://northstar-www.dartmouth.edu/doc/idl/html_6.2/Value_Representations.html
        var element = this.elements[tag];
        var vr ='';
        ...
        if(vr === "US")
        {
            return this.uint16(tag, index);
        }
        return "VR value - ( " + vr + " ) parsing not support";
    };
...
var dicomParser = (function (dicomParser)
{
    ...
    dicomParser.readDicomElementExplicit = function(byteStream)
    {
        ...
        // todo nerijus
      //  console.log('tag2', element.tag);

      //  if(element.tag.indexOf('x0002') > -1)
      //  {
        //    element.vr = byteStream.readFixedString(2);
      //  }
      /*  if(element.tag == 'x00080005' || element.tag == 'x00080008')
        {
            element.vr = 'CS';
        }
        if(element.tag == 'x00080012')
        {
            element.vr = 'DA';
        }
*/
...
var dicomParser = (function (dicomParser)
{
    ...
    dicomParser.readDicomElementImplicit = function(byteStream)
    {
        ...
        var vr = dicomParser.dicomDictionary[element.tag];
        if(vr !== undefined)
        {
            element.vr = vr;
        }
...
var dicomParser = (function (dicomParser)
{
    ...
    dicomParser.dicomDictionary = MD.dicomDictionary;
