...
/** @brief Implementation of ReportIface for <tt>$pacs='dcm4chee-arc-5'</tt>. */
class PacsPartReport extends ReportAbstract implements ReportIface
{
	public function collectReports($studyUid, $withAttachments = false)
	{
		...
		$sql = 'SELECT pk, study_iuid, study_id, study_desc, study_date, study_time, accession_no, notes, pat_birthdate,' .
				" pat_id, pat_fn, pat_gn, ref_phys_fn, ref_phys_gn, $modalitiesExpr AS mods_in_study" .
			' FROM (' .
				'SELECT study.pk, study_iuid, study_id, study_desc, study_date, study_time, accession_no,' .
					" $studynotesExpr AS notes, pat_birthdate, pat_id, pat_fn, pat_gn, ref_phys_fn," .
					' ref_phys_gn, modality' .
				' FROM study' .
				' LEFT JOIN studynotes ON study.pk=studynotes.study_fk' .
				...
				' WHERE study.pk=' . $authDB->sqlEscapeString($studyUid) .
				' GROUP BY study.pk, study_iuid, study_id, study_desc, study_date, study_time, accession_no,' .
					" $studynotesExpr, pat_birthdate, pat_id, pat_fn, pat_gn, ref_phys_fn, ref_phys_gn, modality" .
				' ORDER BY study_date DESC, study_time DESC, pk DESC' .
			")$tableAlias1" .
			' GROUP BY pk, study_iuid, study_id, study_desc, study_date, study_time, accession_no, notes, pat_birthdate,' .
				' pat_id, pat_fn, pat_gn, ref_phys_fn, ref_phys_gn' .
		...
		if ($row = $authDB->fetchAssoc($rs))
		{
			...
			$return["referringphysician"] = $cs->utf8Encode($this->shared->buildPersonName($row['ref_phys_fn'], $row['ref_phys_gn']));
			$return["readingphysician"] = '';
			...
			$return["patientname"] = $cs->utf8Encode($this->shared->buildPersonName($row['pat_fn'], $row['pat_gn']));
			$return["patientbirthdate"] = $this->shared->cleanDbString((string) $row["pat_birthdate"]);
			$return["patienthistory"] = "";
			$return["modality"] = $this->shared->cleanDbString((string) $row['mods_in_study']);
