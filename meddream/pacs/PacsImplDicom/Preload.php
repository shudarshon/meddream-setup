...
/** @brief Implementation of PreloadIface for <tt>$pacs='DICOM'</tt>. */
class PacsPartPreload extends PreloadAbstract implements PreloadIface
{
	...
	private function compareSeries2($a, $b)
	{
		/* compared to QrAbstract::compareSeries, our arrays contain an additional level
	...
	public function fetchInstance($imageUid, $seriesUid, $studyUid)
	{
		...
		$pa = $this->qr->fetchImage($imageUid, $seriesUid, $studyUid);
		if (strlen($pa['error']))
			$return = false;
		else
			$return = $pa['path'];
	...
	/* removeFetchedFile() is not required as this PACS has a managed cache. The default
	   implementation in PreloadAbstract is quite sufficient.
	 */
}
