...
/** @brief Implementation of SearchIface for <tt>$pacs='DICOM'</tt>. */
class PacsPartSearch extends SearchAbstract implements SearchIface
{
	...
	public function findStudies($actions, $searchCriteria, $fromDate, $toDate, $mod, $listMax)
	{
		...
		for ($i = 0; $i < count($searchCriteria); $i++)
		{
			...
			{
				$errorMessage = '[DICOM] Searches by Reading Physician are not supported';
			...
			{
				$errorMessage = '[DICOM] Searches by Source AE Title are not supported';
				$audit->log(false, $auditMsg);
				$this->log->asErr($errorMessage);
				return array('count' => 0, 'error' => $errorMessage);
			}

			$errorMessage = "[DICOM] Searches by '$criteriaName' are not supported";
			$audit->log(false, $auditMsg);
			$this->log->asErr($errorMessage);
			return array('count' => 0, 'error' => $errorMessage);
		}
		...
		{
			$audit->log(false, $auditMsg);
			return array('count' => 0, 'error' => 'Please select only one modality');
		}
