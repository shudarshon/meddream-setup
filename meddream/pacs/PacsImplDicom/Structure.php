...
/** @brief Implementation of StructureIface for <tt>$pacs='DICOM'</tt>. */
class PacsPartStructure extends StructureAbstract implements StructureIface
{
	public function instanceGetMetadata($instanceUid, $includePatient = false)
	{
		...
		{
			$rtn = $this->gw->instanceGetMetadata($instanceUid, $includePatient, $this->commonData['retrieve_entire_study'] != 0);
			if (!strlen($rtn['error']))
			{
				$path = $rtn['path'];
				if (strlen($rtn['path']) && @!file_exists($path))
		...
		$pa = $this->qr->fetchImage($ids[0], $ids[1], $ids[2]);
		if (strlen($pa['error']))
	...
	public function seriesGetMetadata($seriesUid)
	{
		if (strlen($this->commonData['pacs_gateway_addr']))
		{
			return $this->gw->seriesGetMetadata($seriesUid, true, $this->commonData['retrieve_entire_study'] != 0);
		}
