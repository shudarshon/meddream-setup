...
class AmfphpLogger {
    ...
    const LOG_FILE_NAME = 'amfphp.log';
    ...
    public static function logMessage($message){
        $fh = @fopen(dirname(__FILE__) . '/../../../log/' . self::LOG_FILE_NAME, 'a');
        if(!$fh){
    ...
    public function filterSerializedRequest($rawData){
        self::logMessage('[' . date('Y-m-d H:i:s') . "] serialized request : \n" . var_export($rawData, true) . "\n");
    }
    ...
    public function filterDeserializedRequest($deserializedRequest){
        self::logMessage('[' . date('Y-m-d H:i:s') . "] deserialized request : \n " . var_export($deserializedRequest, true));
    }
    ...
    public function filterDeserializedResponse($deserializedResponse){
        self::logMessage('[' . date('Y-m-d-H:i:s') . "] deserialized response : \n " . var_export($deserializedResponse, true));
    }
    ...
    public function filterSerializedResponse($rawData){
        self::logMessage('[' . date('Y-m-d H:i:s') . "] serialized response : \n" . var_export($rawData, true) . "\n");
    }
